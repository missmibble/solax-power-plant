    ...
    constructor(scope, id, props) {
        ...
        this.dashboardApiFunctionAlias = this.newLambdaFunction(
            ...
            deploymentConfig,
            {
                ...
                // Same pattern, for GET /settings-optimizer-settings ("Full
                // automation" toggle). Unlike the dryRun default above, the safe
                // default here is false, so this checks for an explicit 'true'
                // rather than defaulting true unless explicitly 'false'.
                ...
                // same Open-Meteo location BatteryControlFunction uses for
                // tomorrow's forecast, but called live on every /battery-status
                // request rather than once nightly, since "now" needs to stay
                // current through the day. Additive: omitted entirely
                // (fetchCurrentWeather then no-ops) if location isn't configured,
                // same guard BatteryControlFunction itself uses. No API key
                // needed — Open-Meteo's forecast API is free/keyless.
                ...(config.location ? {
        ...
        // (same pattern as ReportFunction's sendEmail flag). The default/'decide'
        // phase (~21:30 Brisbane) checks tomorrow's forecast, decides tonight's
        // chargeUpperSoc, and — if current SOC already exceeds that target —
        // engages VPP SOC Target Control to discharge the surplus instead of
        // setting Self Use mode immediately (see BatteryControlFunction.js). The
        // 'exitDischarge' phase (~23:55, before the 00:00 overnight charge window)
        // is a no-op unless that discharge actually happened tonight, in which
        // case it exits VPP mode and only then sets Self Use mode with tonight's
        // chargeUpperSoc — SOC Target Control has no built-in duration, so this is
        // the only thing that hands control back before the overnight window needs
        // it. dryRun defaults true — see BatteryControlFunction.js. Skipped when
        // config.location/config.batteryControl aren't configured — there'd be no
        // coordinates for the forecast lookup or baseline settings to resend. No
        // weather API key needed (Open-Meteo).
        if (config.location && config.batteryControl) {
            ...
            new events.Rule(this, 'BatteryControlExitDischargeScheduleRule', {
                ruleName: 'powerplant-battery-control-exit-discharge-schedule',
                schedule: events.Schedule.expression(config.lambda.batteryControlFunction.exitDischargeSchedule),
                targets: [new targets.LambdaFunction(this.batteryControlFunctionAlias, {
                    event: events.RuleTargetInput.fromObject({ phase: 'exitDischarge' })
                })]
            });
        ...
        // Nightly (was weekly — see docs/settings-optimizer-logic.md). Compares
        // the hardcoded battery-charge defaults (chargeUpperSocSunny/PartlyCloudy/
        // Overcast) against a rolling lookbackDays (default 7) window of
        // BatteryControlFunction's own decision history via Bedrock, and — only
        // when the effective autoApply is true (config default, dashboard-
        // overridable — see the "Full automation" toggle) — writes recommended
        // values into the same dashboard-editable settings-override row that
        // function already reads. Unlike ReportFunction's/BatteryControlFunction's
        ...
        if (config.settingsOptimizer && config.bedrock?.modelId && config.batteryControl) {
            ...
            // Reads BATTERY_CONTROL# history and the settings-override row, and
            // writes its own status record plus (when autoApply) that same
            // override row.
        ...
        // SettingsOptimizerFunction's latest nightly AI recommendation — read-only,
        // same Lambda again. See DashboardApiFunction.handleSettingsOptimization.
        const settingsOptimizationResource = this.api.root.addResource('settings-optimization');
        settingsOptimizationResource.addMethod(
            'GET',
            new apigateway.LambdaIntegration(this.dashboardApiFunctionAlias),
            protectedMethodOptions
        );

        // Dashboard-editable "Full automation" (autoApply) toggle for
        // SettingsOptimizerFunction — the "self-directing" on/off switch. See
        // DashboardApiFunction.handleGetSettingsOptimizerSettings/handlePutSettingsOptimizerSettings.
        const settingsOptimizerSettingsResource = this.api.root.addResource('settings-optimizer-settings');
        settingsOptimizerSettingsResource.addMethod(
            'GET',
            new apigateway.LambdaIntegration(this.dashboardApiFunctionAlias),
            protectedMethodOptions
        );
        ...
        // DashboardApiFunction.handleGetHouseholdSettings/handlePutHouseholdSettings.
        const householdSettingsResource = this.api.root.addResource('household-settings');
        householdSettingsResource.addMethod(
            'GET',
            new apigateway.LambdaIntegration(this.dashboardApiFunctionAlias),
            protectedMethodOptions
        );
