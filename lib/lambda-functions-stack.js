    ...
    constructor(scope, id, props) {
        ...
        this.dashboardApiFunctionAlias = this.newLambdaFunction(
            ...
            deploymentConfig,
            {
                ...
                // Same pattern, for GET /grid-discharge-settings.
                GRID_DISCHARGE_DEFAULT_ENABLED: String(config.gridDischarge?.enabled ?? true),
                ...
                // automation" toggle). Unlike the dryRun defaults above, the safe
        ...
        // BatteryControlFunction.js. Skipped when config.location/config.batteryControl
        // aren't configured — there'd be no coordinates for the forecast lookup
        // or baseline settings to resend. No weather API key needed (Open-Meteo).
        ...
        // the hardcoded battery-charge and grid-discharge defaults
        // (chargeUpperSocSunny/PartlyCloudy/Overcast, fallbackReservePercent/
        // safetyMarginPercent) against a rolling lookbackDays (default 7) window of
        // BatteryControlFunction's/GridDischargeFunction's own decision history via
        // Bedrock, and — only when the effective autoApply is true (config default,
        // dashboard-overridable — see the "Full automation" toggle) — writes
        // recommended values into the same dashboard-editable settings-override rows
        // those two functions already read. Unlike ReportFunction's/BatteryControlFunction's
        ...
        // Dashboard-editable enabled/dry-run toggle for GridDischargeFunction —
        // same on/off + control-mode pattern as battery-settings above. See
        // DashboardApiFunction.handleGetGridDischargeSettings/handlePutGridDischargeSettings.
        const gridDischargeSettingsResource = this.api.root.addResource('grid-discharge-settings');
        gridDischargeSettingsResource.addMethod(
            'GET',
            new apigateway.LambdaIntegration(this.dashboardApiFunctionAlias),
            protectedMethodOptions
        );
