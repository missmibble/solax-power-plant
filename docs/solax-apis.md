...
- **Battery:** `deviceSn`, `dataTime`, `plantLocalTime`, `registerNo`, `deviceStatus`, `batterySOC`, `batteryRemainings` (kWh), `batterySOH`, `chargeDischargePower` (+charge/−discharge, W), `batteryVoltage`, `batteryCurrent`, `batteryTemperature`, `batteryCycleTimes`, `totalDeviceCharge`/`totalDeviceDischarge` (kWh) — an earlier version of this doc claimed the first of these was misspelled `totalDevicCharge`; verified against a live API call that it's actually spelled correctly (matching `totalDeviceDischarge`)
