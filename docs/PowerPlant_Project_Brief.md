...
- The client library (`lambda/Utilities/solax-client.js`) implements the read/monitoring endpoints (Auth, Information Management, Monitoring Management), plus one control/write endpoint: §7 Inverter Work Mode Control's Self Use mode (`batch_set_spontaneity_self_use`), used by `BatteryControlFunction` — see Automated battery control below. Every other control/write endpoint in `docs/solax-apis.md` (EMS work modes, VPP remote control, export/import limits, EV charger control, battery heating) is out of scope — §9 VPP Remote Control was previously implemented here for `GridDischargeFunction`'s grid-export arbitrage, removed once that opportunity was confirmed unavailable — see Grid discharge arbitrage below.
...
7. **Battery Control Lambda** — triggered nightly by EventBridge, before the overnight grid-charge window. Checks tomorrow's weather forecast (Open-Meteo, no API key required) and adjusts the inverter's grid-charge target (`chargeUpperSoc`) accordingly — the app's first control/write call to SolaX Cloud, dry-run by default. The charge percentages and a real on/off kill switch are dashboard-editable, overriding the static config. Before each decision it also has Bedrock judge the previous night's decision against what actually happened (usage, not just weather) — see `docs/battery-charge-logic.md`.
8. **SSM Parameter Store** — holds the SolaX OAuth Client ID/Secret as SecureString parameters, provisioned by the CDK stack itself (via a custom resource, since CloudFormation can't create SecureString parameters natively) rather than committed to the repo. The weather forecast (Open-Meteo) needs no credential, so it isn't part of this.
...
6. **Automated battery control** — implemented, dry-run by default: `BatteryControlFunction` checks tomorrow's weather forecast (Open-Meteo) nightly and adjusts the Inverter's grid-charge target (`chargeUpperSoc`, via `batch_set_spontaneity_self_use` — a control/write endpoint, the first this app calls) — sunny forecast → 40% (enough to cover morning usage until solar recharges it); overcast/rainy or ambiguous forecast → 100% (safe default). `config.batteryControl.dryRun` defaults `true` — logs and emails what it *would* do without ever calling the control endpoint. Full decision logic, thresholds, and worked examples for review before enabling live: `docs/battery-charge-logic.md`.
7. **Grid discharge arbitrage** — implemented, then fully removed: the retailer was believed to offer a premium 27c/kWh feed-in rate 5-9pm (vs. a flat 2c/kWh the rest of the day), and `GridDischargeFunction` discharged surplus battery capacity to the grid via VPP SOC Target Control to capture it — removed once that premium feed-in opportunity was confirmed unavailable for this account.
8. **Settings optimizer** — implemented, recommendation-only by default: the tuning defaults above (`chargeUpperSocSunny`/`chargeUpperSocPartlyCloudy`/`chargeUpperSocOvercast`) were each set once and never revisited — `SettingsOptimizerFunction` runs nightly, aggregates a rolling week of `BatteryControlFunction`'s own decision-accuracy history, and asks Bedrock whether any should change, bounded by a minimum sample size and a maximum per-run adjustment regardless of what the model recommends. `config.settingsOptimizer.autoApply` defaults `false` — emails a recommendation without changing anything until enabled. Full logic and known risks: `docs/settings-optimizer-logic.md`.
