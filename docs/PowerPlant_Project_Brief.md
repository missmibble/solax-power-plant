...
- The client library (`lambda/Utilities/solax-client.js`) implements the read/monitoring endpoints (Auth, Information Management, Monitoring Management), plus exactly one control/write endpoint: §7 Inverter Work Mode Control's Self Use mode (`batch_set_spontaneity_self_use`), used by `BatteryControlFunction` — see Automated battery control below. Every other control/write endpoint in `docs/solax-apis.md` (EMS work modes, VPP remote control, export/import limits, EV charger control, battery heating) is still out of scope, since this app otherwise recommends battery configuration changes rather than applying them automatically.
...
4. **Web dashboard** — S3 + CloudFront, static SPA (`dashboard/`) charting PV/battery/import/export like the manual reports we've reviewed, in its own stack (`DashboardStack`) so its S3 origin and the API Gateway origin can share one CloudFront distribution without a cross-stack dependency cycle.
...
7. **Battery Control Lambda** — triggered nightly by EventBridge, before the overnight grid-charge window. Checks tomorrow's weather forecast and adjusts the inverter's grid-charge target (`chargeUpperSoc`) accordingly — the app's first control/write call to SolaX Cloud, dry-run by default. See `docs/battery-charge-logic.md`.
...
2. **Dashboard** — done — `DashboardApiFunction` (`GET /readings?range=day|week`, includes battery data) plus a static S3+CloudFront frontend (`dashboard/`, `DashboardStack`) that calls it same-origin, with CloudFront injecting the API key so it never reaches the browser.
